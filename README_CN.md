...
```bash
python rm_border.py -i ./project/ -r --output_dir ./output/
```
...
### v1.1.0 (2026-05-13)
- 🐛 修复：RGBA 图片所有像素完全不透明时 `--background` 参数失效的问题
- ✨ 新增：`--auto` (`-a`) 智能边界识别，自动扫描四边纯色段并裁剪
- ✨ 新增：`--tolerance` (`-t`)、`--recursive` (`-r`)、`--format` (`-f`) 简写参数
